...
export async function GET(req: Request) {
  try {
    ...
    const items = fs.readdirSync(/*turbopackIgnore: true*/ resolved, { withFileTypes: true }).map(item => ({
      ...
      path: path.join(/*turbopackIgnore: true*/ resolved, item.name).replace(/\\/g, "/")
    })).sort((a, b) => {
...
export async function POST(req: Request) {
  try {
    ...
    const { action, target, content } = body || {};
    
    if (!target) return NextResponse.json({ error: "Missing target path" }, { status: 400 });
    
    if (action === 'read') {
      if (!fs.existsSync(/*turbopackIgnore: true*/ target) || !fs.statSync(/*turbopackIgnore: true*/ target).isFile()) return NextResponse.json({ error: "File not found" }, { status: 404 });
      return NextResponse.json({ content: fs.readFileSync(/*turbopackIgnore: true*/ target, "utf8") });
    } else if (action === 'save') {
      try {
      ...
      try {
        if (fs.statSync(/*turbopackIgnore: true*/ target).isDirectory()) {
